...
$(document).ready(function(){
  $('.chat-icon').click(function(){
      if( !(/buy_credits|page3\.php/.test(window.location.href)) ) {
          if( $('#invitations-widget').is(':visible')) {
              $('#invitations-widget').slideUp();
          }else{
              $('#invitations-widget').slideDown();
          }
      }
  });
  
	$('.hamburger').on('show.bs.dropdown', function () {

        $('#mainmenu').css('width', '250px');
        $('#submenu-mob').css('width', '100%');
		...
		if(windowHeight<$('.sidebar-menu-mob').height()){ 
			$('.sidebar-menu-mob').css('min-height',windowHeight);
			$('.scrollable-menu').css('max-height',windowHeight);
		}
	});
	$('.hamburger').on('hide.bs.dropdown', function () {
	...
	$('.dropdown-submenu a.morelink').on("click", function(e){   	
		$(this).next('ul').toggle();

		if($(this).next('ul').is(':visible')) {
            $('.morelink').html(' More<i class="fa fa-caret-up"></i>');
            $('.sidebar-menu-mob').animate({ scrollTop: "250px" });
		}
		else {
			$('.morelink').html(' More<i class="fa fa-caret-down"></i>');
		}
		e.stopPropagation(); 
		e.preventDefault();											
  });	
...
//GALLERY
$('.gallery-wrap:not(.gallery-exclude)').each(function() { // the containers for all your galleries
    $(this).magnificPopup({
...
(function($) {   
    $(function() {
        var member_home_jcarousel = $('.member-home-jcarousel');

        member_home_jcarousel
